...
The <<scoring,score-function>> allows to rank the documents given the coefficients of the estimated model
...
The `linreg_error` score function allows you to rank documents that are retrieved by a query
by the error term regarding a previously estimated linear regression model.
For each search result document the error term is computed by multiplying values of
the defined numerical fields
with given regression coefficients.
If used on a multi-valued field, only the first value of the field is used in calculations.

Assuming again the data consists of documents representing sold house prices with features
 like number of bedrooms, bathrooms and size etc. we estimate the linear regression
 model by using the `linreg_stats` aggregation a got the
 can let predict or validate
 the price for our house in Morro Bay with 2000 square feet, 4 bedrooms and 2 bathrooms by:
...
== License
Copyright 2018 Scaleborn UG (haftungsbeschränkt).
