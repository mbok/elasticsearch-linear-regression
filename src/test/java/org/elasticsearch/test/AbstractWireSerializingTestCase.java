...
public abstract class AbstractWireSerializingTestCase<T extends Writeable> extends ESTestCase {
  ...
  public void testEqualsAndHashcode() throws IOException {
    for (int runs = 0; runs < NUMBER_OF_TEST_RUNS; runs++) {
      ...
      assertThat("same instance's hashcode returns different values if called multiple times",
   ...
   *
   * Override this method if you need to register {@link NamedWriteable}s for the test object to
   * de-serialize.
   *
   * By default this will return a {@link NamedWriteableRegistry} with no registered {@link
   * NamedWriteable}s
   */
